...
export class EmployeeTree {
    ...
    private readonly EmployeeRetriever: EmployeeRetriever;
    ...
    constructor(private listEmployees: Employee[]) {
        ...
        this.postorderTraversalBuiltTree();
    }

    // Traverse and build the employee tree during initialization
    private postorderTraversalBuiltTree(): void {
        ...
        for (const rootEmployee of rootEmployees) {
            // Build the subtree for the current root employee using postorder traversal
            const rootNode = this.postorderTraversalRecursive(rootEmployee);
            if (rootNode) {
        ...
        // Build the employee tree nodes with postorder recursive approach
    private postorderTraversalRecursive(employee: Employee): EmployeeNode | null {
        // Find all edges (employees reporting to the current employee)
        ...
        for (const directReport of edges) {
            // Build subtree for the current direct report using postorder traversal
            const childNode = this.postorderTraversalRecursive(directReport);
            if (childNode) {
        ...
        // Add the child nodes to the current employee's node
